...
object Environment extends PluginDefaults {
  ...
  lazy val aws = OsgiProject("aws", imports = Seq("!org.jclouds.compute.*", "!org.jclouds.compute", "*")) dependsOn (Core.exception, Core.workspace, Core.batch, gridscale, sge, ssh) settings
    (libraryDependencies ++= Seq(Libraries.gridscaleAWS, Libraries.gridscaleSGE))
